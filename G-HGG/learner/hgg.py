...
class TrajectoryPool:
  # ring buffer of trajectories
	...
	def pad(self):
    # repeat current pool state as padding
		if self.counter>=self.length:
...
class MatchSampler:
    ...
    # make a pool of initial states plus noise
    ...
    # get distance between 2 different goals in graph
		if self.args.graph:
	...
	def sample(self, idx):
      ...
      # goes here but why? just for the sake of adding noise?
    ...
    # get some achieved trajectories
		...
		achieved_value = []
    # for each achieved trajectory
		for i in range(len(achieved_pool)):
      # make an observation from the initial state and the goal
...
class HGGLearner:
	...
	def learn(self, args, env, env_test, agent, buffer, write_goals=0):
		...
		for i in range(args.episodes):
      # reset the ith environment and get the first observation (initial state)
      ...
      # update the sampler
		...
		for i in range(args.episodes):
      # get the observation from the ith environment
			...
			# sampler goal may not be in goal space
			explore_goals.append(explore_goal)
			# generate actual goals from the environment
			test_goals.append(self.env.generate_goal())

			# Perform HER training by interacting with the environment
			# set the goal to be the explore goal, either a sampled goal or the desired goal
			self.env_List[i].goal = explore_goal.copy()
			# if you need to make a list of goals, add explore goal to goal list
			if write_goals != 0 and len(goal_list)<write_goals:
				goal_list.append(explore_goal.copy())
			# get the observation from the env list
			obs = self.env_List[i].get_obs()
			# create a trajectory
			current = Trajectory(obs)
			# IMPORTANT STEP HERE create a trajectory of achieved goals
			...
			for timestep in range(args.timesteps):
				...
				# IMPORTANT STEP HERE append the achieved goal to the trajectory
				trajectory.append(obs['achieved_goal'].copy())
				if timestep==args.timesteps-1: done = True
				current.store_step(action, obs, reward, done)
				if done: break
			# IMPORTANT STEP HERE add to list of achieved trajectoryies
		...
		# add trajectories that moved to the achieved trajectory pool
