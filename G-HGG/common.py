...
def get_args():
	...
	parser.add_argument('--temp', help='temperature', type=np.float32, default=0.01)
	...
	parser.add_argument('--buffer_type', help='type of replay buffer / whether to use Energy-Based Prioritization', type=str, default='energy', choices=['normal','energy','mep'])
	...
	# parser.add_argument('--mep', help='use mep', type=bool, default=False)
	parser.add_argument('--rank_method', help='rank method', type=str, default='dense', choices=['none', 'average', 'min', 'max', 'dense', 'ordinal'])
	...
	logger_name = args.alg+'-'+args.env+'-'+args.learn+'-'+time.strftime("%d-%m-%Y_%H-%M-%S")
...
def experiment_setup(args):
  # Returns the environment, a test environment, the agent, buffer, learner, and tester
	env = make_env(args)
