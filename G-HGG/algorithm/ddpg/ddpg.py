...
class DDPG:
	...
	def create_model(self):
		...
		def create_network():
      # @tf.keras.utils.register_keras_serializable(package='Custom', name='mep')
      # def mep_regularizer(weight_matrix):
      #   np.power(weight_matrix, 1/(temp+1e-2))
      #   p_traj = 
			def mlp_policy(obs_ph):
				with tf.variable_scope('net', initializer=tf.contrib.layers.xavier_initializer()):
					...
					pi = tf.layers.dense(pi_dense3, self.args.acts_dims[0], activation=tf.nn.tanh, name='pi') #, activity_regulatizer=mep_regularizer if self.args.mep else None)
				return pi
